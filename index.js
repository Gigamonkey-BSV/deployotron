...
module.exports = app => {
  ...
  app.on(['pull_request.closed', ], async context => {
    if(context.payload.pull_request.merged && context.payload.pull_request.base.ref==="deploy") {
      app.log(context.payload);

      // Probot API note: context.repo() => { username: 'hiimbex', repo: 'testing-things' }
      const res = await context.github.repos.createDeployment(context.repo({
        ref: context.payload.pull_request.base.ref, // The ref to deploy. This can be a branch, tag, or SHA.
        ...
        description: 'Triggering a push', // Short description of the deployment
      ...
      const newDirectory=directory+context.payload.repository.name;
      fs.removeSync(newDirectory);
      console.log("Cloning "+context.payload.repository.svn_url + "into " + newDirectory);
      ...
      clone(context.payload.repository.svn_url, newDirectory,cloneOptions).then(function(repository) {
          console.log("Checked out " + context.payload.pull_request.base.sha +" to "+newDirectory);
      }) .catch((err) => {
        console.error(err);
      });
